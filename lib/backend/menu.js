...
let menuPopularityCache = null;
let menuPopularityCacheAt = 0;

/** Đếm số lần đặt theo id/tên món từ đơn hoàn tất (90 ngày gần nhất). */
export async function getMenuPopularityScores() {
  if (menuPopularityCache && Date.now() - menuPopularityCacheAt < 300000) return menuPopularityCache;
  const byId = {};
  const byName = {};
  try {
    ...
    if (data.length < 2) {
      menuPopularityCache = { byId, byName };
      menuPopularityCacheAt = Date.now();
      return menuPopularityCache;
    }
    const cmap = ordersColumnMap(data[0]);
    for (let i = 1; i < data.length; i++) {
      const st = String(ordersCell(data[i], cmap, 'Trạng thái', 11) || '').trim();
      if (!ordersCompletedStatus(st)) continue;
      let items = [];
      try {
        const raw = ordersCell(data[i], cmap, 'Các món', 5);
        items = typeof raw === 'string' && raw ? JSON.parse(raw) : Array.isArray(raw) ? raw : [];
      } catch {
        items = [];
      }
      items.forEach((it) => {
        const qty = parseInt(it.quantity, 10) || parseInt(it.qty, 10) || 1;
        const id = String(it.id || '').trim();
        const name = String(it.name || it.itemName || '').trim();
        if (id) byId[id] = (byId[id] || 0) + qty;
...
export async function getMenu() {
  ...
  const pop = await getMenuPopularityScores();
  result.forEach((item) => {
    const idScore = pop.byId[String(item.id || '')] || 0;
    const nameScore = pop.byName[String(item.name || '').trim()] || 0;
    item.orderScore = Math.max(idScore, nameScore);
  });
  result.sort((a, b) => {
    const diff = (b.orderScore || 0) - (a.orderScore || 0);
    if (diff) return diff;
...
export async function saveMenuItem(data) {
  ...
  CACHE.clear('menu');
  CACHE.clear('portal_bootstrap_v1');
  return id;
}
