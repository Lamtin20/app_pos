...
export async function officeStreakConfig() {
  ...
  const streakDays = parseInt(cfg.streakDays, 10) || 4;
  const rewardWeekday = parseInt(cfg.rewardWeekday, 10);
  const rewardDow = Number.isFinite(rewardWeekday) ? rewardWeekday : 5;
  const rewardLabel = WEEKDAY_LABELS[rewardDow] || 'Thứ 6';
  return {
    active: !!p && (p.active !== false),
    memberVisible: cfg.memberVisible !== false,
    title: String(cfg.memberTitle || p?.name || 'Chuỗi đặt văn phòng').trim(),
    memberDescription: String(cfg.memberDescription || '').trim(),
    memberConditions: String(cfg.memberConditions || '').trim(),
    messageProgress: String(cfg.messageProgress || '').trim(),
    messageComplete: String(cfg.messageComplete || '').trim(),
...
const STREAK_DAY_LABELS = ['T2', 'T3', 'T4', 'T5'];
...
export async function getOfficeStreakProgressForPhone(phoneRaw) {
  const cfg = await officeStreakConfig();
  const phone = normalizeVnPhoneDigits(phoneRaw);
  if (!cfg.active || cfg.memberVisible === false || !phone || phone.length < 9) {
    return {
      active: false,
      streak: 0,
  ...
  const days = STREAK_DAY_LABELS.map((label, idx) => {
    const weekday = idx + 1;
    return {
      label,
      weekday,
