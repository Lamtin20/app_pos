...
import { safeNum, formatDateTz, normalizeVnPhoneDigits } from '../format.js';
...
async function validateFeedbackVoucher(code, orderAmount) {
  ...
  for (let i = rows.length - 1; i >= 1; i--) {
    ...
    const disc = Math.round((parseFloat(orderAmount) || 0) * pct / 100);
    return {
      valid: true,
      discount: disc,
...
/** Đánh dấu voucher cá nhân (GY góp ý hoặc TV thành viên mới) đã dùng. */
...
export async function issueWelcomeMemberVoucher(phone) {
  ...
  const now = new Date();
  const expires = new Date(now.getTime() + WELCOME_MEMBER_VOUCHER_DAYS * 24 * 3600 * 1000);
  const pct = WELCOME_MEMBER_VOUCHER_PCT;
...
export async function validatePromo(code, orderAmount, customerPhone, cartJson) {
  ...
  if (rtEarly === 'group_order' || rtEarly === 'feedback_reward') {
    return {
      valid: false,
      message: 'Đây là mã cấu hình hệ thống (đơn nhóm / voucher góp ý) - hệ thống tự áp dụng, không nhập trực tiếp.',
    };
  }
  ...
  if (rt === 'combo_qty_tiers') {
    ...
    const discountTier = Math.round((orderAmount * bestP) / 100);
    return {
      valid: true,
...
export async function getCheckoutPromoOptions(phone, cartJson, subtotal) {
  ...
  const options = [];
  const seen = {};
  const sug = await getSuggestedPromosWithContext(p, '', cartStr, sub, false);
  for (const s of sug.suggestions || []) {
    ...
    const v = await validatePromo(s.code, sub, p, cartStr);
    if (!v.valid) continue;
    seen[s.code] = true;
    options.push({
      code: s.code,
      title: s.name,
      subtitle: s.reason || v.message,
      kind: 'promo',
      discount: v.discount,
    });
  }
  ...
  for (const pr of promos) {
    ...
    const v = await validatePromo(pr.code, sub, p, cartStr);
    if (!v.valid) continue;
    seen[pr.code] = true;
    options.push({
      code: pr.code,
  ...
  const personal = await getPersonalVouchersForPhone(p);
  for (const pv of personal) {
    if (!pv?.code || seen[pv.code]) continue;
    const v = await validatePromo(pv.code, sub, p, cartStr);
    if (!v.valid) continue;
    seen[pv.code] = true;
    options.push({
      code: pv.code,
      title: pv.title || pv.code,
      subtitle: (v.message || '') + (pv.expires ? ` · HSD ${pv.expires}` : ''),
      kind: 'voucher',
      discount: v.discount,
    });
  }
...
export async function activeRulePromo(ruleType) {
  ...
  let active = null;
  for (const p of promos) {
    if (String(p.ruleType || '').trim().toLowerCase() !== ruleType) continue;
    found = true;
    if (active) continue;
