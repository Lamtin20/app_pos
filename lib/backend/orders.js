...
async function groupSave(row, status, data, orderId) {
  ...
  CACHE.set(`grp_${state.groupId}`, state, 3);
  return state;
}
...
export async function joinGroupOrder(groupId, memberId, name, guestPhone) {
  ...
  return withLock('group_orders', async () => {
    ...
    if (state.status === 'locked' && !groupMemberIdsMatch(state.hostPhone, mid))
...
export async function syncGroupCart(groupId, memberId, name, items, promoCode, discount, clearCart) {
  ...
  return withLock('group_orders', async () => {
    ...
    const sanitized = groupSanitizeItems(items);
    let members = state.members.map(groupMemberRow);
    let idx = members.findIndex((m) => groupMemberIdsMatch(m, mid));
    if (idx < 0) {
      if (!/^0\d{9}$/.test(mid)) throw new Error('Đăng nhập SĐT để đồng bộ đơn nhóm');
      members.push({
        id: mid,
        phone: mid,
        name: String(name || '').slice(0, 40) || `KH ${mid.slice(-4)}`,
        items: sanitized,
        promoCode: promo,
        discount: disc,
        updatedAt: new Date().toISOString(),
      ...
      const nextItems =
        sanitized.length === 0 && !wipeOk && (cur.items || []).length > 0 ? cur.items : sanitized;
      members[idx] = {
        id: cur.id || mid,
        phone: cur.phone || mid,
        name: String(name || cur.name || '').slice(0, 40) || cur.name || `KH ${mid.slice(-4)}`,
        items: nextItems,
        promoCode: promo,
        discount: disc,
        updatedAt: new Date().toISOString(),
    ...
    const data = { groupName: state.groupName, members };
    return groupSave(found.row, state.status, data, null);
  });
}
  ...
  return withLock('group_orders', async () => {
    ...
    const data = {
      ...
      members: state.members
        .filter((m) => !groupMemberIdsMatch(m, mid))
        .map(groupMemberRow),
    };
    if (groupMemberIdsMatch(state.hostPhone, mid)) {
...
export async function setGroupOrderLock(groupId, memberId, locked) {
  ...
  return withLock('group_orders', async () => {
    ...
    if (!found) throw new Error(`Không tìm thấy nhóm ${code}`);
    const state = await resolveGroupExpiry(found);
    if (!groupMemberIdsMatch(state.hostPhone, mid)) throw new Error('Chỉ trưởng nhóm mới khóa/mở nhóm được');
