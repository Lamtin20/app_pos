...
import { addLoyaltyPoints, redeemPoints, saveCustomer } from './customers.js';
import { applyPromoCode, markFeedbackVoucherUsed, groupDiscountPct, feedbackRewardPct } from './promotions.js';
...
export async function saveOrder(orderData) {
  ...
  if (orderData.promoCode) await markFeedbackVoucherUsed(orderData.promoCode, orderId);
  if (phoneCol && phoneCol.length >= 9) {
    await saveCustomer({
      phone: phoneCol,
      name: String(orderData.customerName || '').trim() || 'Khách hàng',
    });
  }
  if (status === 'Hoàn thành') {
    try {
      await processOrderCompletion(orderId, { ...orderData, items: itemsSafe });
    } catch {
      /* log */
    }
  }
...
export async function savePickupOnlineOrder(orderData) {
  ...
  await saveCustomer({
    phone: phoneCol,
    name: orderData.customerName || resolved.name || 'Khách hàng',
  });
  if (promoCode) await markFeedbackVoucherUsed(promoCode, orderId);
  CACHE.clear(['dashboard', 'cashflow_month', 'cashflow_today', 'cashflow_week']);
  return {
    ok: true,
    orderId,
...
export async function confirmOrderInOrdersSheet(orderId) {
  const sheetName = await getOrdersSheet();
...
export async function checkBankTransferPayment(orderId, expectedAmount) {
  const lockResult = await tryWithLock('bank_check', async () => {
    ...
    if (oData.length >= 2) {
      ...
      for (let r = 1; r < oData.length; r++) {
        ...
        ordersFinal = Math.round(
          parseFloat(String(oData[r][cFinal >= 0 ? cFinal : 8]).replace(/,/g, '')) || 0,
        );
        break;
      }
    }
    ...
    const colContent = ordersHeaderPick(headers, ['Nội dung thanh toán', 'Nội dung CK', 'Mô tả giao dịch'], -1);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const transactionAmount = Math.round(
        parseFloat(String(row[colAmount] || '').toString().replace(/,/g, '')) || 0,
      );
      ...
      if ((memoOk || kOk) && transactionAmount > 0) {
        const proc = bankTransferRowProcessed(currentProcessingStatus);
    ...
    const allTotal = allMatchRows.reduce((s, x) => s + x.amount, 0);

    if (unprocessedRows.length) {
      let hit = unprocessedRows.find((x) => Math.abs(x.amount - exp) <= 5);
      if (!hit && unpaidTotal >= exp - 5 && unprocessedRows.length === 1) hit = unprocessedRows[0];
      if (hit) {
        await updateCell(bankSheetName, hit.sheetRow, colProcessingStatus + 1, 'Đã xử lý');
        const ok = await confirmOrderInOrdersSheet(orderId);
        return ok
          ? { status: 'paid', amountPaid: hit.amount, expectedAmount: exp }
          : { status: 'error', message: 'Không xác nhận được đơn hàng' };
      }
    }
    ...
    if (unprocessedRows.length && unpaidTotal >= exp - 5) {
      ...
      const ok = await confirmOrderInOrdersSheet(orderId);
      return ok
        ? { status: 'paid', amountPaid: unpaidTotal, expectedAmount: exp }
        : { status: 'error', message: 'Không xác nhận được đơn hàng' };
    }

    if (allTotal >= exp - 5) {
      const ok2 = await confirmOrderInOrdersSheet(orderId);
      if (ok2) return { status: 'paid', amountPaid: allTotal, expectedAmount: exp, recovered: true };
    }
  ...
  if (lockResult?.busy) return { status: 'pending', message: 'busy' };
  return lockResult;
}
    ...
    if (String(data[i][cmap['Mã ĐH']] || data[i][0] || '').trim() === oid) {
      ...
      if (ordersCompletedStatus(st)) {
        try {
          ...
          await processOrderCompletion(oid, {
...
async function groupStateFromRow(vals) {
  ...
  const members = (data.members || []).map((m) => {
    ...
    return {
      ...
      name: String(m.name || ''),
      items,
      subtotal: groupMemberSubtotal(items),
...
export async function createGroupOrder(hostPhone, hostName, groupName) {
  ...
  return withLock('group_orders', async () => {
    ...
    const data = {
      groupName: gname,
      members: [{ id: phone, phone, name, items: [], updatedAt: now.toISOString() }],
    };
...
export async function joinGroupOrder(groupId, memberId, name, guestPhone) {
  ...
  return withLock('group_orders', async () => {
    ...
    let me = data.members.find((m) => m.id === mid);
    if (!me) {
      if (data.members.length >= GROUP_MAX_MEMBERS)
        throw new Error(`Nhóm đã đủ ${GROUP_MAX_MEMBERS} người`);
      data.members.push({
        id: mid,
        phone: isGuest ? groupNormPhone(guestPhone) : mid,
...
export async function syncGroupCart(groupId, memberId, name, items) {
  const code = groupNormCode(groupId);
  const mid = groupNormMemberId(memberId);
  if (!code) throw new Error('Thiếu mã nhóm');
  return withLock('group_orders', async () => {
    ...
    const data = {
      ...
      members: state.members.map((m) => ({
        ...
        phone: m.phone,
        name: m.name,
        items: m.id === mid ? groupSanitizeItems(items) : m.items,
        updatedAt: m.id === mid ? new Date().toISOString() : m.updatedAt,
      })),
    };
...
export async function finalizeGroupOrder(groupId, hostPhone, checkout) {
  ...
  return withLock('group_orders', async () => {
    ...
    const groupDisc = groupPct > 0 ? Math.round((subtotal * groupPct) / 100) : 0;
    const promoDisc = parseFloat(co.discount) || 0;
    const discount = Math.max(groupDisc, promoDisc);
    const usedGroupDisc = groupDisc > 0 && groupDisc >= promoDisc;
    const promoCode = usedGroupDisc ? '' : co.promoCode || '';
    const membersWithItems = state.members.filter((m) => (m.items || []).length);
    const noteExtra =
      `[GROUP:${code}] «${state.groupName}» - ${membersWithItems.length} người: ` +
    ...
    const newState = await groupSave(found.row, 'ordered', data, res.orderId);
    res.group = newState;
    return res;
  });
}
