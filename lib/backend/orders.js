...
import { safeNum, normalizeVnPhoneDigits, parseOrderDate, roundDiscountToThousand } from '../format.js';
import {
  getOrdersSheet,
  ordersColumnMap,
  ordersCell,
...
export async function saveOrder(orderData) {
  ...
  if (status === 'Hoàn thành') {
    try {
      ...
      const { recordOfficeStreakOrder } = await import('./officeStreak.js');
      await recordOfficeStreakOrder(phoneCol, orderTime, itemsSafe);
    } catch {
      /* log */
    }
  }
...
export async function savePickupOnlineOrder(orderData) {
  ...
  try {
    const { recordOfficeStreakOrder } = await import('./officeStreak.js');
    await recordOfficeStreakOrder(phoneCol, new Date(), itemsSafe);
  } catch {
    /* streak optional */
  }
  CACHE.clear(['dashboard', 'cashflow_month', 'cashflow_today', 'cashflow_week']);
...
export async function checkBankTransferPayment(orderId, expectedAmount) {
  const lockResult = await tryWithLock('bank_check', async () => {
    ...
    if (ordersStatus && !orderStatusIsAwaitingBank(ordersStatus)) {
      ...
      return { status: 'pending', message: 'not_awaiting_bank' };
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if ((memoOk || kOk) && transactionAmount > 0) {
        ...
        allMatchRows.push({ sheetRow: i + 2, amount: transactionAmount, processed: proc, memoMatch: true });
        if (!proc) unprocessedRows.push({ sheetRow: i + 2, amount: transactionAmount, memoMatch: true });
      }
    }
    ...
    if (unprocessedRows.length) {
      ...
      if (!hit && unprocessedRows.length === 1 && unprocessedRows[0].memoMatch) hit = unprocessedRows[0];
...
async function finalizeGroupOrderCore(found, state, checkout, opts = {}) {
  ...
  try {
    ...
    for (const m of state.members) {
      ...
      const mph = groupNormPhone(m.phone || m.id);
      if (/^0\d{9}$/.test(mph)) await recordOfficeStreakOrder(mph, orderTime, m.items);
    }
  } catch {
    /* streak optional */
  }
...
export async function syncGroupCart(groupId, memberId, name, items, promoCode, discount) {
  const code = groupNormCode(groupId);
  const mid = groupNormMemberId(memberId);
  if (!code) throw new Error('Thiếu mã nhóm');
  ...
  return withLock('group_orders', async () => {
    ...
    const data = {
      ...
      members: state.members.map((m) => ({
        ...
        promoCode: m.id === mid ? promo : (m.promoCode || ''),
        discount: m.id === mid ? disc : (parseFloat(m.discount) || 0),
