...
export async function issueSunRedeemVoucher(phone, sunCost, maxAmount, validDays, menuItem) {
  ...
  const mi = menuItem && typeof menuItem === 'object' ? menuItem : {};
  const menuId = String(mi.id || '').trim();
  const menuName = String(mi.name || '').trim();
  const menuImage = String(mi.imageUrl || mi.coverImageUrl || '').trim();
...
/** Đổi Sun → voucher 1 ly (hạn 30 ngày mặc định). menuItem: { id, name, imageUrl, basePrice? } */
export async function redeemSunForVoucher(phoneRaw, program, menuItem) {
  ...
  const mi = menuItem && typeof menuItem === 'object' ? menuItem : null;
  if (!mi?.id) return { ok: false, message: 'Chọn món nước trước khi đổi Sun' };
  ...
  const drinkPrice = Math.max(1000, parseFloat(mi.basePrice ?? mi.price) || sunRedeemMaxAmount(program));
...
export async function validateSunRedeemVoucher(code, orderAmount, itemsJson) {
  ...
  for (let i = rows.length - 1; i >= 1; i--) {
    ...
    const menuItemId = String(rows[i][8] || '').trim();
    const menuItemName = String(rows[i][9] || '').trim();
    let items = [];
    try {
    ...
    if (menuItemId) {
      let match = 0;
      items.forEach((it) => {
        if (String(it.id || '') !== menuItemId) return;
        const unit = parseFloat(it.price) || 0;
        const qty = parseInt(it.qty, 10) || parseInt(it.quantity, 10) || 1;
        const line = unit * qty;
        if (unit > 0 && line > 0) match = Math.min(line, maxAmt);
      });
      if (!match) {
        return {
          valid: false,
          message: menuItemName
            ? `Voucher chỉ áp dụng cho «${menuItemName}» — thêm đúng món vào giỏ nhé!`
            : 'Thêm đúng món đã chọn khi đổi Sun vào giỏ để dùng voucher!',
        };
      }
      return {
        valid: true,
        discount: match,
        message: `Voucher đổi Sun — Miễn phí ${menuItemName || '1 ly'} (giảm ${match.toLocaleString('vi-VN')}đ)`,
      };
    }
    let best = 0;
    items.forEach((it) => {
...
export async function getSunRedeemVouchersForPhone(phone) {
  ...
  for (let i = rows.length - 1; i >= 1; i--) {
    ...
    if (!code) continue;
    const menuItemName = String(rows[i][9] || '').trim();
    const menuItemImage = String(rows[i][10] || '').trim();
    out.push({
      code,
      maxAmount: parseFloat(rows[i][2]) || SUN_REDEEM_DEFAULT_MAX,
      kind: 'sun_redeem',
      title: menuItemName ? `Voucher đổi Sun — ${menuItemName}` : 'Voucher đổi Sun (1 ly)',
      menuItemId: String(rows[i][8] || '').trim(),
      menuItemName,
      menuItemImage,
