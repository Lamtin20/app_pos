...
import { getPromotions, issueWelcomeMemberVoucher } from './promotions.js';
import { lookupMembershipByPhone } from './membership.js';
import { saveCustomer, getCustomerByPhone, isGenericCustomerName } from './customers.js';

function clearMemberPhoneCaches(...phones) {
  const keys = ['customers_v1', 'Customers_ext_cols'];
  phones.forEach((p) => {
    const ph = normalizeVnPhoneDigits(p);
    if (!ph) return;
    keys.push(
      `member_profile_${ph}`,
      `portal_lookup_${ph}`,
      `member_notifications_${ph}`,
      `membership_lookup_${ph}`
    );
  });
  CACHE.clear(keys);
}
...
async function memberCodeExists(code, data, headers) {
  const ix = (headers || []).indexOf('Mã HV');
  if (ix < 0) return false;
  const rows = data || (await getDataRange('Customers'));
  const hdrs = headers || (rows[0] || []).map((h) => String(h || '').trim());
  const col = hdrs.indexOf('Mã HV');
  if (col < 0) return false;
  const cu = String(code || '').trim().toUpperCase();
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][col] || '').trim().toUpperCase() === cu) return true;
  }
  return false;
}
  ...
  for (let i = 1; i < data.length; i++) {
    if (normalizeVnPhoneDigits(data[i][ixPhone]) === phone) {
      ...
      for (let t = 0; t < 12; t++) {
        ...
        if (!(await memberCodeExists(code, data, headers))) break;
      }
      if (ixCode >= 0) await updateCell('Customers', i + 1, ixCode + 1, code);
      clearMemberPhoneCaches(phone);
      return code;
    }
  }
...
export async function getMemberProfile(phoneRaw, skipCache) {
  const phone = normalizeVnPhoneDigits(phoneRaw);
  if (!phone || phone.length < 9) throw new Error('SĐT không hợp lệ');
  const cacheKey = `member_profile_${phone}`;
  if (!skipCache) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (normalizeVnPhoneDigits(data[i][headers.indexOf('SĐT')]) === phone) {
      ...
      if (!skipCache) CACHE.set(cacheKey, out, 60);
      return out;
    }
  }
  const out = {
...
export async function saveMemberProfile(phoneRaw, patch) {
  ...
  if (rowIdx < 0) {
    ...
    if (!nm) throw new Error('Vui lòng nhập tên của bạn');
    if (isGenericCustomerName(nm)) throw new Error('Vui lòng nhập tên thật của bạn');
    await saveCustomer({ phone, name: nm });
    return getMemberProfile(phone);
  }
  const row = data[rowIdx - 1].slice();
  if (p.name != null) {
    const incoming = String(p.name).trim();
    const existingNm = String(row[ix('Tên')] || '').trim();
    if (incoming && (!isGenericCustomerName(incoming) || isGenericCustomerName(existingNm))) {
      row[ix('Tên')] = incoming;
    }
  }
  ...
  clearMemberPhoneCaches(phone);
  return getMemberProfile(phone, true);
}
...
export async function changeMemberPhone(oldPhoneRaw, newPhoneRaw) {
  ...
  clearMemberPhoneCaches(oldPhone, newPhone);
  return getMemberProfile(newPhone, true);
}
...
const MEMBER_PORTAL_CMS_MAX_BYTES = 9000;
const MEMBER_CMS_ZONES = {
...
export const MEMBER_MEDIA_SPECS = {
  carouselMobile: '750×300px (5:2, JPG/WebP)',
  carouselDesktop: '680×272px (5:2, JPG/WebP — mỗi ảnh trong carousel 2 banner)',
  carousel: '750×300px mobile · 680×272px desktop (5:2)',
  categoryBannerMobile: '750×150px (5:1, JPG/WebP)',
  categoryBannerDesktop: '1200×240px (5:1, JPG/WebP)',
  categoryBanner: '750×150px mobile · 1200×240px desktop (5:1)',
  heroLogin: '900×1200px (dọc, JPG/WebP)',
};

function memberCmsParseDate(v) {
  if (!v) return null;
  let s = String(v).trim();
  if (!s) return null;
...
function normalizeMemberCmsItem(raw, idx) {
  const o = raw && typeof raw === 'object' ? raw : {};
  let zone = String(o.zone || 'carousel').trim();
  if (!MEMBER_CMS_ZONES[zone]) zone = 'carousel';
  return {
...
function migrateLegacyBannersToCmsItems(banners) {
  if (!Array.isArray(banners) || !banners.length) return [];
  return banners.map((b, i) =>
    normalizeMemberCmsItem(
      {
        id: `legacy_car_${i}`,
        zone: 'carousel',
        enabled: true,
...
async function getMemberPortalCmsItemsRaw(skipCache) {
  if (!skipCache) {
    const cc = CACHE.get('member_portal_cms_items');
    if (cc) return cc;
  }
  let items = [];
  const raw = await getProperty(MEMBER_PORTAL_CMS_KEY);
  if (raw) {
    try {
  ...
  items = items.map((it, i) => normalizeMemberCmsItem(it, i));
  if (!items.length) {
    const rawMain = await getProperty(MEMBER_PORTAL_PROP_KEY);
    if (rawMain) {
      try {
        const mainObj = JSON.parse(rawMain);
        if (mainObj && Array.isArray(mainObj.banners) && mainObj.banners.length) {
          items = migrateLegacyBannersToCmsItems(mainObj.banners);
        }
      } catch {
        /* ignore */
      }
...
function isMemberCmsItemActive(item, now) {
  if (!item || item.enabled === false) return false;
  if (item.perpetual) return true;
  const t = now?.getTime ? now.getTime() : Date.now();
  const start = memberCmsParseDate(item.startAt);
  const end = memberCmsParseDate(item.endAt);
  if (start && t < start.getTime()) return false;
  if (end && t > end.getTime()) return false;
  return true;
}

function filterActiveMemberCmsItems(items, now) {
  const n = now || new Date();
  return (items || [])
    .filter((it) => isMemberCmsItemActive(it, n) && String(it.image || '').trim())
    .sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
}

function cmsItemsToBannerSlides(items) {
  return (items || []).map((it) => ({
...
function applyMemberPortalCmsToSettings(settings, cmsAll, forAdmin) {
  const merged = settings || {};
  const all = Array.isArray(cmsAll) ? cmsAll : [];
  if (forAdmin) {
    merged.cmsItems = all;
    return merged;
  }
  const active = filterActiveMemberCmsItems(all);
  const byZone = {};
  active.forEach((it) => {
    if (!byZone[it.zone]) byZone[it.zone] = [];
    byZone[it.zone].push(it);
  });
  const carouselLegacy = byZone.carousel || [];
  const carouselMobile = byZone.carousel_mobile?.length ? byZone.carousel_mobile : carouselLegacy;
  const carouselDesktop = byZone.carousel_desktop?.length ? byZone.carousel_desktop : carouselLegacy;
  if (carouselMobile.length) merged.bannersMobile = cmsItemsToBannerSlides(carouselMobile);
  if (carouselDesktop.length) merged.bannersDesktop = cmsItemsToBannerSlides(carouselDesktop);
  ...
  else if (carouselLegacy.length) merged.banners = cmsItemsToBannerSlides(carouselLegacy);
  else if (!Array.isArray(merged.banners) || !merged.banners.length) {
    merged.banners = cmsItemsToBannerSlides(migrateLegacyBannersToCmsItems(merged.banners || []));
  }
  const heroLogin = (byZone.hero_login || [])[0];
  if (heroLogin?.image) merged.heroImageUrl = heroLogin.image;
  const heroReg = (byZone.hero_register || [])[0];
  if (heroReg?.image) merged.heroRegisterImageUrl = heroReg.image;
  const menuBg = (byZone.menu_hero_bg || [])[0];
  if (menuBg?.image) merged.menuHeroBgUrl = menuBg.image;
  return merged;
}

async function saveMemberPortalCmsItems(items) {
  const list = (Array.isArray(items) ? items : [])
    .map((it, i) => normalizeMemberCmsItem(it, i))
    .filter(
      (it) =>
        String(it.image || '').trim() ||
        String(it.title || '').trim() ||
        String(it.badge || '').trim()
    );
  const str = JSON.stringify(list);
  if (str.length > MEMBER_PORTAL_CMS_MAX_BYTES) {
    throw new Error(
      `Danh sách banner CMS quá dài (~${Math.round(str.length / 1024)}KB, tối đa ~9KB).`
    );
  }
  await setProperty(MEMBER_PORTAL_CMS_KEY, str);
  CACHE.clear(['member_portal_cms_items', 'member_portal_settings', 'portal_bootstrap_v1']);
  return { ok: true, count: list.length };
}
...
export async function getMemberPortalSettings(skipCache, forAdmin) {
  ...
  const defaults = {
    ...
    categoryBanners: [],
    mediaSpecs: {
      carouselMobile: MEMBER_MEDIA_SPECS.carouselMobile,
      carouselDesktop: MEMBER_MEDIA_SPECS.carouselDesktop,
      carousel: MEMBER_MEDIA_SPECS.carousel,
      categoryBannerMobile: MEMBER_MEDIA_SPECS.categoryBannerMobile,
      categoryBannerDesktop: MEMBER_MEDIA_SPECS.categoryBannerDesktop,
      categoryBanner: MEMBER_MEDIA_SPECS.categoryBanner,
      heroLogin: MEMBER_MEDIA_SPECS.heroLogin,
    },
    memberTiers: defaultMemberTiers(),
    redeemPrograms: defaultRedeemPrograms(),
  ...
  merged.redeemPrograms = normalizeRedeemPrograms(merged.redeemPrograms);
  merged.menuCategories = normalizeMenuCategories(merged.menuCategories);
  const cmsAll = await getMemberPortalCmsItemsRaw(skipCache || forAdmin);
  merged = applyMemberPortalCmsToSettings(merged, cmsAll, !!forAdmin);
...
export function defaultMenuCategories() {
  return [
    { name: 'Sữa hạt', enabled: true },
    { name: 'Mix hạt', enabled: true },
    { name: 'Coffee', enabled: true },
    { name: 'Topping', enabled: true },
    { name: 'Matcha', enabled: true },
    { name: 'Trà trái cây', enabled: true },
  ];
}

export function normalizeMenuCategories(raw) {
  const base = defaultMenuCategories();
  ...
  raw.forEach((it) => {
    ...
    out.push({
      name,
      enabled: it.enabled !== false && it.enabled !== 'false' && it.enabled !== 0,
    });
  });
  return out.length ? out : base;
}

export function enabledMenuCategoryNames(categories) {
  return normalizeMenuCategories(categories)
    .filter((c) => c.enabled)
    .map((c) => c.name);
}

export function defaultRedeemPrograms() {
  return [
    {
      sun: 10,
      title: '1 ly nước bất kỳ',
      desc: 'Đổi 10 Sun lấy 1 ly nước bất kỳ trên menu — voucher có hiệu lực 30 ngày.',
      icon: 'fa-mug-hot',
      featured: true,
      maxAmount: 65000,
      validDays: 30,
...
export function normalizeRedeemPrograms(raw) {
  const list = Array.isArray(raw) ? raw : [];
  const out = list
    .map((p) => ({
      sun: Math.max(1, parseInt(p?.sun, 10) || 0),
      title: String(p?.title || '').trim().slice(0, 80),
      desc: String(p?.desc || '').trim().slice(0, 200),
      icon: String(p?.icon || 'fa-gift').trim().slice(0, 40),
      featured: p?.featured === true || p?.featured === 'true' || p?.featured === 1,
      maxAmount: Math.max(1000, parseFloat(p?.maxAmount) || 65000),
      validDays: Math.max(1, parseInt(p?.validDays, 10) || 30),
    }))
    .filter((p) => p.sun > 0 && p.title);
  return out.length ? out : defaultRedeemPrograms();
}

export function defaultMemberTiers() {
  return [
    {
      id: 'member', name: 'Thành viên', minSun: 0, promoteSun: 0, sunPerCup: 1, redeemSun: 10,
      redeemDesc: '1 ly nước bất kỳ', multiplier: 1,
      benefits: [
        { icon: 'fa-sun', text: 'Tích 1 Sun mỗi ly hoàn tất' },
        { icon: 'fa-gift', text: 'Đổi 10 Sun = 1 ly nước bất kỳ (voucher 30 ngày)' },
      ],
    },
    {
      id: 'silver', name: 'Bạc', minSun: 10, promoteSun: 10, sunPerCup: 1, redeemSun: 10,
      redeemDesc: '1 ly nước bất kỳ', multiplier: 1,
      benefits: [
    ...
    {
      id: 'gold', name: 'Vàng', minSun: 60, promoteSun: 50, sunPerCup: 1, redeemSun: 10,
      redeemDesc: '1 ly nước bất kỳ', multiplier: 1,
      benefits: [
        { icon: 'fa-cake-candles', text: 'Tặng 01 phần bánh sinh nhật' },
    ...
    {
      id: 'diamond', name: 'Kim cương', minSun: 210, promoteSun: 150, sunPerCup: 1, redeemSun: 10,
...
export function normalizeMemberTiers(raw) {
  ...
  return defs.map((d) => {
    ...
    return {
      ...
      promoteSun: parseInt(found.promoteSun, 10) >= 0 ? parseInt(found.promoteSun, 10) : (parseInt(found.minSun, 10) >= 0 ? parseInt(found.minSun, 10) : d.promoteSun),
...
export async function saveMemberPortalSettings(obj) {
  ...
  if (obj.redeemPrograms) obj.redeemPrograms = normalizeRedeemPrograms(obj.redeemPrograms);
  if (obj.menuCategories) obj.menuCategories = normalizeMenuCategories(obj.menuCategories);
  const cmsItems = obj.cmsItems;
  const toSave = { ...obj };
  ...
  if (Array.isArray(cmsItems)) await saveMemberPortalCmsItems(cmsItems);
  CACHE.clear(['member_portal_settings', 'member_portal_cms_items', 'portal_bootstrap_v1']);
  return { ok: true };
}
...
export function computeMemberTierProgress(cupsRaw, settingsTiers) {
  ...
  const tiers = tierSrc.map((t) => ({
    id: t.id,
    name: t.name,
    min: parseInt(t.minSun, 10) || 0,
    promoteSun: parseInt(t.promoteSun, 10) || 0,
  }));
  ...
  const rule = next ? (next.promoteSun > 0 ? next.promoteSun : next.min - current.min) : 0;
  return {
    cups: c,
    ...
    nextTierId: next ? next.id : '',
    nextTierMin: next ? next.min : null,
    progressPct,
    cupsToNext,
    tierHint: next
      ? `Còn ${cupsToNext} Sun để lên ${next.name}`
...
export async function getMemberOffersPayload(phoneRaw) {
  ...
  const { getPersonalVouchersForPhone } = await import('./promotions.js');
  const personal = await getPersonalVouchersForPhone(phone);
  const promos = personal.map((v) => {
    const kind = String(v.kind || '').trim();
    if (kind === 'sun_redeem') {
      return {
        code: v.code,
        name: v.menuItemName || 'Voucher đổi Sun',
        type: 'fixed',
        value: v.maxAmount,
        minOrder: 0,
        description: v.title || 'Voucher đổi Sun',
        conditions: v.menuItemName
          ? `Áp dụng cho món «${v.menuItemName}» · HSD ${v.expires || ''}`
          : `HSD ${v.expires || ''}`,
        endDate: v.expires,
        kind: 'sun_redeem',
        imageUrl: v.imageUrl || v.menuItemImage || '',
        menuItemId: v.menuItemId || '',
        menuItemName: v.menuItemName || '',
      };
    }
    if (kind === 'office_streak') {
      return {
        code: v.code,
        name: v.title || 'Voucher chuỗi văn phòng',
        type: 'fixed',
        value: v.maxAmount,
        minOrder: 0,
        description: v.title || 'Voucher chuỗi văn phòng',
        conditions: `HSD ${v.expires || ''}`,
        endDate: v.expires,
        kind: 'office_streak',
      };
    }
    if (kind === 'feedback' || kind === 'welcome') {
      return {
        code: v.code,
        name: v.title || (kind === 'welcome' ? 'Voucher thành viên mới' : 'Voucher góp ý'),
        type: 'percent',
        value: v.percent,
        minOrder: 0,
        description: v.title || 'Voucher của bạn',
        conditions: `HSD ${v.expires || ''}`,
        endDate: v.expires,
        kind,
      };
    ...
    return {
      code: v.code,
      name: v.title || 'Voucher',
      type: 'fixed',
      value: v.maxAmount || v.percent || 0,
      minOrder: 0,
      description: v.title || 'Voucher của bạn',
      conditions: `HSD ${v.expires || ''}`,
      endDate: v.expires,
  ...
  const { getOfficeStreakProgressForPhone } = await import('./officeStreak.js');
  const officeStreak = await getOfficeStreakProgressForPhone(phone);
  return {
    ok: true,
    ...
    redeemPrograms: normalizeRedeemPrograms(settings.redeemPrograms),
  };
}

export async function memberPortalBootstrapPayload() {
  const cacheKey = 'portal_bootstrap_v1';
  const cached = CACHE.get(cacheKey);
  if (cached) return cached;
  try {
    const result = {
...
export async function memberPortalLookup(phoneRaw) {
  const phone = normalizeVnPhoneDigits(phoneRaw);
  const cacheKey = phone ? `portal_lookup_${phone}` : '';
  if (cacheKey) {
    const cached = CACHE.get(cacheKey);
    if (cached) return cached;
  }
  try {
    ...
    if (cacheKey && profile && String(profile.name || '').trim()) {
...
export async function registerMemberPortalCustomer(obj) {
  ...
  clearMemberPhoneCaches(phone);
  let voucher = null;
  if (!existed) voucher = await issueWelcomeMemberVoucher(phone);
  return { ok: true, isNew: !existed, phone, name: nm, voucher };
}
...
/** Khách tự đổi Sun → voucher 1 ly (theo chương trình trong Cổng HV). */
export async function redeemMemberSunProgram(phoneRaw, sunCost, menuItemId) {
  const phone = normalizeVnPhoneDigits(phoneRaw);
  if (!phone || phone.length < 9) return { ok: false, message: 'SĐT không hợp lệ' };
  const cost = parseInt(sunCost, 10);
  if (!cost || cost < 1) return { ok: false, message: 'Thiếu số Sun cần đổi' };
  const menuItemKey = String(menuItemId || '').trim();
  if (!menuItemKey) return { ok: false, message: 'Chọn món nước trước khi đổi Sun' };
  const settings = await getMemberPortalSettings(false);
  const programs = normalizeRedeemPrograms(settings.redeemPrograms);
  const program = programs.find((p) => p.sun === cost);
  if (!program) return { ok: false, message: 'Chương trình đổi Sun không hợp lệ' };
  const { getMenu } = await import('./menu.js');
  const menu = await getMenu();
  const item = (menu || []).find((m) => String(m.id) === menuItemKey);
  if (!item || item.available === false) return { ok: false, message: 'Món không hợp lệ hoặc đã tắt' };
  const cat = String(item.category || '').toLowerCase();
  if (cat.indexOf('topping') >= 0) return { ok: false, message: 'Voucher đổi Sun không áp dụng cho topping' };
  const { redeemSunForVoucher } = await import('./sunRedeem.js');
  const r = await redeemSunForVoucher(phone, program, {
    id: item.id,
    name: item.name,
    imageUrl: item.imageUrl || item.coverImageUrl || '',
    basePrice: item.basePrice,
  });
  if (r?.ok) clearMemberPhoneCaches(phone);
  return r;
}
