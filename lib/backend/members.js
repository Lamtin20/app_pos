...
function customerRowToProfile(row, headers) {
  ...
  return {
    ...
    addresses: addrJson,
    locked: { email: !!lock.email, dob: !!lock.dob || !!dobStr, phone: !!lock.phone || true },
    memberCode: '',
  };
}
...
export function computeMemberTierProgress(cupsRaw) {
  const tiers = [
...
export async function getMemberOffersPayload(phoneRaw) {
  ...
  const tierProgress = computeMemberTierProgress(profile.points);
  const now = Date.now();
  const promos = [];
