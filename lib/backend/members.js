...
export async function getMemberOffersPayload(phoneRaw) {
  ...
  const sunVouchers = await getSunRedeemVouchersForPhone(phone);
  for (const uv of sunVouchers) {
    if (!uv?.code) continue;
    promos.push({
      code: uv.code,
      name: uv.menuItemName || 'Voucher đổi Sun',
      type: 'sun_redeem',
      value: uv.maxAmount,
      minOrder: 0,
      description: uv.title || 'Voucher đổi Sun',
      conditions: uv.menuItemName
        ? `Áp dụng cho món «${uv.menuItemName}» · HSD ${uv.expires || ''}`
        : `HSD ${uv.expires || ''}`,
      endDate: uv.expires,
      kind: 'sun_redeem',
      imageUrl: uv.imageUrl || uv.menuItemImage || '',
      menuItemId: uv.menuItemId || '',
