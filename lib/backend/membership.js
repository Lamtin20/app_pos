...
import { safeNum, normalizeVnPhoneDigits, parseOrderDate, formatDateTz, parseVnAmount } from '../format.js';
...
function membershipParsePromoCell(raw) {
  ...
  s.split(/[|;,\n]+/).forEach((part) => {
    ...
    if (!token) return;
    const m = token.match(/^([a-z_]+)\s*:\s*(.+)$/i);
    if (!m) {
      if (/^[a-z][a-z0-9_]*$/i.test(token)) out.scenarios.push(token);
      return;
    }
...
function membershipRowToPkg(r) {
  return {
    ...
    ribbonLabel: String(r[11] || '').trim(),
    imageUrl: String(r[12] || '').trim(),
    listPrice: safeNum(r[13]),
  };
}
...
export async function getMembershipPackages() {
  ...
  const out = rows
    ...
    .map((p) => {
      ...
      const listPrice = Math.round(parseFloat(p.listPrice) || 0);
      const savingsFromList = listPrice > quote.finalPrice ? listPrice - quote.finalPrice : 0;
      const savings = savingsFromList > 0 ? savingsFromList : Math.max(0, quote.discount || 0);
      return {
        ...p,
...
export async function lookupMembershipByPhone(phoneRaw) {
  try {
    ...
    const cacheKey = `membership_lookup_${phone}`;
    const cached = CACHE.get(cacheKey);
    if (cached) return cached;
    const sheetName = await membershipSubSheet();
    const rows = await getDataRange(sheetName);
    if (rows.length < 2) {
      const empty = { ok: true, list: [] };
      CACHE.set(cacheKey, empty, 45);
      return empty;
    }
...
export async function registerMembershipSubscription(obj) {
  ...
  const province = String(obj.province != null && obj.province !== '' ? obj.province : '').trim();
  const ward = String(obj.ward != null && obj.ward !== '' ? obj.ward : '').trim();
  ...
  const schedIn = obj.regSchedule || obj.schedule;
  let schedJson = '';
  if (schedIn != null && schedIn !== '') {
    schedJson = typeof schedIn === 'string' ? schedIn : JSON.stringify(schedIn);
  }
  const favFromSched = (() => {
    if (!schedIn || typeof schedIn !== 'object') return '';
    const dd = schedIn.dailyDrinks;
    if (dd && typeof dd === 'object') {
      const keys = ['T2', 'T3', 'T4', 'T5', 'T6'];
      for (const k of keys) {
        const v = String(dd[k] || '').trim();
        if (v) return v;
      }
    }
    return String(schedIn.defaultDrink || '').trim();
  })();
  const rowMap = {
    'Mã ĐK': subId,
    SĐT: phone,
    ...
    'Món yêu thích': String(obj.favoriteDrink || favFromSched || '').trim(),
    'Lịch giao KH': schedJson,
  };
  const sheetName = await membershipSubSheet();
  await appendRow(sheetName, await membershipBuildSubRow(rowMap));
  await saveCustomer({ phone, name });
  CACHE.clear([`membership_lookup_${phone}`, `portal_lookup_${phone}`]);
  return {
    ok: true,
    subId,
...
export async function adminListMembershipPackages() {
  const sheetName = await membershipPkgSheet();
  const rows = await getDataRange(sheetName);
  return rows
    .slice(1)
...
export async function adminDeleteMembershipPackage(pkgId) {
  const id = String(pkgId || '').trim();
  if (!id) throw new Error('Thiếu Mã gói');
  const subSheet = await membershipSubSheet();
  const subRows = await getDataRange(subSheet);
  for (let i = 1; i < subRows.length; i++) {
    if (!String(subRows[i][0] || '').trim()) continue;
    if (String(subRows[i][3] || '').trim() === id) {
      throw new Error('Không xóa được: đã có khách đăng ký gói này. Hãy đặt Hiển thị = Không.');
    }
  }
  const sheetName = await membershipPkgSheet();
  const all = await getDataRange(sheetName);
...
export async function adminSaveMembershipPackage(obj) {
  ...
  const vals = [
    ...
    pkgKind,
    promoCell,
    String(o.ribbonLabel || o['Nhãn ribbon'] || '').trim(),
    String(o.imageUrl || o['Ảnh gói'] || '').trim(),
    parseFloat(o.listPrice ?? o['Giá niêm yết']) || 0,
  ];
  const all = await getDataRange(sheetName);
  let updated = false;
  for (let i = 1; i < all.length; i++) {
...
export async function checkMembershipBankPayment(subId, expectedAmount) {
  return tryWithLock('membership_bank', async () => {
    ...
    const colContent = (() => {
      const picked = ordersHeaderPick(headers, ['Nội dung thanh toán', 'Nội dung CK'], -1);
    ...
    const colProcessingStatus = ordersHeaderPick(headers, ['Trạng thái xử lý', 'Trạng thái xử lý CK'], -1);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const proc = hasProcCol ? bankTransferRowProcessed(row[colProcessingStatus]) : false;
      ...
      if ((memoOk || kOk) && transactionAmount > 0 && !proc) {
        unprocessedRows.push({ sheetRow: i + 2, amount: transactionAmount, memoMatch: !!memoOk });
      }
    }
    if (!unprocessedRows.length) return { status: 'pending' };
    const unpaidTotal = unprocessedRows.reduce((s, x) => s + x.amount, 0);
    const hit =
      unprocessedRows.find((x) => x.memoMatch && Math.abs(x.amount - exp) <= 5) ||
      unprocessedRows.find((x) => x.memoMatch) ||
      (unprocessedRows.length === 1 ? unprocessedRows[0] : null);
    if (hit || unpaidTotal >= exp - 5) {
      const rowsToMark = hit ? [hit] : unprocessedRows;
      for (const row of rowsToMark) {
        if (hasProcCol) await updateCell(bankSheetName, row.sheetRow, colProcessingStatus + 1, 'Đã xử lý');
      }
      await activateMembershipSubscription(subId);
      return { status: 'paid', amountPaid: hit ? hit.amount : unpaidTotal, expectedAmount: exp };
    }
