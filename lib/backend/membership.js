...
export async function getMembershipPackages() {
  ...
  const rows = await getDataRange(sheetName);
  const out = rows.slice(1).filter((r) => String(r[0] || '').trim()).map(membershipRowToPkg);
...
export async function adminSaveMembershipPackage(obj) {
  const sheetName = await membershipPkgSheet();
  const all = await getDataRange(sheetName);
  const headers = all[0] || MEMBERSHIP_PKG_HEADERS;
  const row = headers.map((h) => (obj[h] !== undefined ? obj[h] : obj[String(h)] !== undefined ? obj[String(h)] : ''));
  const pkgId = String(obj['Mã gói'] || obj.id || '').trim();
  let updated = false;
  for (let i = 1; i < all.length; i++) {
    if (String(all[i][0] || '').trim() === pkgId) {
      await updateRow(sheetName, i + 1, row);
      updated = true;
      break;
    }
  }
  if (!updated) await appendRow(sheetName, row);
