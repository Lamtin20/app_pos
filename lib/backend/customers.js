...
function clearCustomerDerivedCaches(phone) {
...
export function isGenericCustomerName(name) {
...
/** Chọn tên lưu sheet: ưu tiên tên thật, không ghi đè bằng placeholder. */
export function pickCustomerNameForSave(orderName, resolvedName, existingName) {
  const order = String(orderName || '').trim();
  const resolved = String(resolvedName || '').trim();
  const existing = String(existingName || '').trim();
  if (order && !isGenericCustomerName(order)) return order;
  if (resolved && !isGenericCustomerName(resolved)) return resolved;
  if (existing && !isGenericCustomerName(existing)) return existing;
...
async function getCustomersFromSheetOnly() {
  ...
  return rows
    ...
    .filter((c) => c.phone && !String(c.phone).startsWith('DEL_'));
}
...
export async function saveCustomer(data) {
  ...
  for (let i = 1; i < existing.length; i++) {
    if (normalizeVnPhoneDigits(existing[i][0]) === phone) {
      ...
      const incoming = data.name != null ? String(data.name).trim() : '';
      const current = String(row9[1] || '').trim();
      const nextName = pickCustomerNameForSave(incoming, '', current);
      if (nextName) row9[1] = nextName;
      row9[8] = data.note != null ? String(data.note) : row9[8];
      await updateRow('Customers', i + 1, row9);
      clearCustomerDerivedCaches(phone);
      return { ok: true, message: 'Đã cập nhật khách hàng!' };
    }
  }
  const newName = pickCustomerNameForSave(data.name, '', '') || 'Khách hàng';
...
export async function deleteCustomer(phoneRaw) {
  await getSheet('Customers', CUSTOMERS_HEADERS);
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (normalizeVnPhoneDigits(cell) === phone) {
      const stamp = 'DEL_' + Date.now() + '_' + phone;
      const row9 = data[i].slice(0, 9);
      while (row9.length < 9) row9.push('');
      row9[0] = stamp;
      await updateRow('Customers', i + 1, row9);
      clearCustomerDerivedCaches(phone);
      CACHE.clear('customers_v1');
      return { ok: true, message: 'Đã xóa khách hàng khỏi danh sách' };
    }
  }
