...
let sheetMapCachedAt = 0;

/** In-memory read cache — cuts duplicate Sheets API reads on hot Vercel instances. */
const dataRangeCache = new Map();
const dataRangeStale = new Map();
const dataRangeInFlight = new Map();

const SHEET_MAP_TTL_MS = 5 * 60 * 1000;
const DEFAULT_READ_TTL_SEC = Math.max(
  15,
  parseInt(process.env.SHEETS_READ_CACHE_TTL_SEC || '90', 10) || 90
);

function isSheetsQuotaError(err) {
  const msg = String(err?.message || err || '').toLowerCase();
  return msg.includes('quota exceeded') || msg.includes('rate limit') || err?.code === 429;
}

function dataRangeCacheKey(sheetName) {
  return `${getSpreadsheetId()}:${sheetName}`;
}

export function invalidateSheetDataCache(sheetName) {
  if (sheetName) {
    const key = dataRangeCacheKey(sheetName);
    dataRangeCache.delete(key);
    dataRangeInFlight.delete(key);
    return;
  }
  dataRangeCache.clear();
  dataRangeInFlight.clear();
}

async function fetchDataRangeFromApi(sheetName) {
...
async function refreshSheetMap(force = false) {
  const ssId = getSpreadsheetId();
  ...
  if (!force && sheetIdByName.size && Date.now() - sheetMapCachedAt < SHEET_MAP_TTL_MS) return;
...
export async function getDataRange(sheetName, opts = {}) {
  const skipCache = opts.skipCache === true;
  const ttlSec = opts.ttlSec != null ? opts.ttlSec : DEFAULT_READ_TTL_SEC;
  const key = dataRangeCacheKey(sheetName);

  if (!skipCache && ttlSec > 0) {
    const hit = dataRangeCache.get(key);
    if (hit && Date.now() < hit.expiresAt) return hit.data;
  }

  if (!skipCache && dataRangeInFlight.has(key)) {
    return dataRangeInFlight.get(key);
  }

  const task = (async () => {
    try {
      const rows = await fetchDataRangeFromApi(sheetName);
      dataRangeStale.set(key, rows);
      if (!skipCache && ttlSec > 0) {
      ...
      const stale = dataRangeStale.get(key);
      if (stale && isSheetsQuotaError(err)) return stale;
      throw err;
    } finally {
      dataRangeInFlight.delete(key);
    }
  ...
  if (!skipCache) dataRangeInFlight.set(key, task);
  return task;
}
